...
async function runTestFile(filePath: string): Promise<TestResult> {
  ...
  try {
    ...
    for (const log of logs) {
      if (log.includes("[GROUP]")) {
        currentGroup = log.replace(/.*\[GROUP\]\s*/, "").trim();
      } else if (log.includes("[CASE]")) {
        currentTest = log.replace(/.*\[CASE\]\s*/, "").trim();
        testStart = Date.now();
      } else if (log.includes("✅ PASS")) {
        result.passed++;
        result.tests.push({
          name: currentGroup ? `${currentGroup} › ${currentTest}` : currentTest,
          status: "pass",
          duration: Math.max(1, Math.random() * 10), // Simulated timing
        });
      } else if (log.includes("❌ FAIL")) {
        result.failed++;
        const errorMsg = log.replace(/.*FAIL:\s*/, "").trim();
        result.tests.push({
          name: currentGroup ? `${currentGroup} › ${currentTest}` : currentTest,
          status: "fail",
