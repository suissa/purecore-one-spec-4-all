...
Uma das maiores barreiras para adotar uma nova ferramenta é o medo de ter que "jogar tudo fora e começar de novo". O one-spec-4-all foi projetado com o princípio oposto em mente.

O Que É a Filosofia Aditiva?

A Filosofia Aditiva significa que o one-spec-4-all funciona como um superset (superconjunto) do Jest. Ele não nega o passado; ele o expande. Se você tem milhares de testes escritos com a sintaxe padrão do Jest (describe, it), não precisa reescrever uma única linha. O framework entende nativamente o seu código legado.

Essa filosofia se manifesta em três benefícios principais:

- Seu código legado continua funcionando: Testes antigos que usam describe e it rodam nativamente no executor do one-spec-4-all. Seu conhecimento e seu código continuam 100% válidos.
...
1. Instale o pacote: Abra seu terminal e execute o seguinte comando para adicionar o one-spec-4-all ao seu projeto.
...
3. Execute o teste: Volte ao seu terminal e rode o executor do one-spec-4-all.
...
import { scenario, to } from "@purecore/one-spec-4-all";
...
import { ensure, verify, that } from "@purecore/one-spec-4-all";
...
Conclusão: Adotando o one-spec-4-all Gradualmente

A mensagem mais importante é que você não precisa parar tudo e reescrever seus testes existentes. Graças à Filosofia Aditiva, o one-spec-4-all coexiste perfeitamente com o Jest.

O código abaixo, com um teste legado e um teste novo no mesmo arquivo, é 100% válido e será executado pelo mesmo comando. Observe que este arquivo é 100% válido e executável pelo one-spec-4-all.
...
O one-spec-4-all não veio para substituir o que já funciona. Ele veio para dar o vocabulário certo para cada tarefa, tornando seus testes mais expressivos, precisos e valiosos. É uma ferramenta de adição, não de substituição, projetada para evoluir com você e sua equipe.
